...
static struct opticflow_result_t opticflow_result[ACTIVE_CAMERAS][NUM_VER_SEC][NUM_HOR_SEC];    ///< The opticflow result

static bool opticflow_got_result[ACTIVE_CAMERAS][NUM_VER_SEC][NUM_HOR_SEC];       ///< When we have an optical flow calculation
...
struct image_t *sections_img_p[NUM_VER_SEC*NUM_HOR_SEC-1]
...
static void opticflow_telem_send(struct transport_tx *trans, struct link_device *dev)
{
  ...
  for (int idx_camera = 0; idx_camera < ACTIVE_CAMERAS; idx_camera++) {
    if (opticflow_result[idx_camera][0][0].noise_measurement < 0.8) {
      pprz_msg_send_OPTIC_FLOW_EST(trans, dev, AC_ID,
                                   &opticflow_result[idx_camera][0][0].fps, &opticflow_result[idx_camera][0][0].corner_cnt,
                                   &opticflow_result[idx_camera][0][0].tracked_cnt, &opticflow_result[idx_camera][0][0].flow_x,
                                   &opticflow_result[idx_camera][0][0].flow_y, &opticflow_result[idx_camera][0][0].flow_der_x,
                                   &opticflow_result[idx_camera][0][0].flow_der_y, &opticflow_result[idx_camera][0][0].vel_body.x,
                                   &opticflow_result[idx_camera][0][0].vel_body.y, &opticflow_result[idx_camera][0][0].vel_body.z,
                                   &opticflow_result[idx_camera][0][0].div_size,
                                   &opticflow_result[idx_camera][0][0].surface_roughness,
                                   &opticflow_result[idx_camera][0][0].divergence,
                                   &opticflow_result[idx_camera][0][0].camera_id); // TODO: no noise measurement here...
    }
  }
...
void opticflow_module_init(void)
{
  ...
  for (int idx_camera = 0; idx_camera < ACTIVE_CAMERAS; idx_camera++) {
    for (int i = 0; i< NUM_VER_SEC; i++) {
      for (int i = 0; j< NUM_HOR_SEC; j++) {
        opticflow_got_result[idx_camera][i][j] = false;
      }
    }  
  }
...
void opticflow_module_run(void)
{
  ...
  for (int idx_camera = 0; idx_camera < ACTIVE_CAMERAS; idx_camera++) {
    if (opticflow_got_result[idx_camera][0][0]) {
      uint32_t now_ts = get_sys_time_usec();
      AbiSendMsgOPTICAL_FLOW(FLOW_OPTICFLOW_ID + idx_camera, now_ts,
                             opticflow_result[idx_camera][0][0].flow_x,
                             opticflow_result[idx_camera][0][0].flow_y,
                             opticflow_result[idx_camera][0][0].flow_der_x,
                             opticflow_result[idx_camera][0][0].flow_der_y,
                             opticflow_result[idx_camera][0][0].noise_measurement,
                             opticflow_result[idx_camera][0][0].div_size);
      ...
      if (opticflow_result[idx_camera][0][0].noise_measurement < 0.8) {
        AbiSendMsgVELOCITY_ESTIMATE(VEL_OPTICFLOW_ID + idx_camera, now_ts,
                                    opticflow_result[idx_camera][0][0].vel_body.x,
                                    opticflow_result[idx_camera][0][0].vel_body.y,
      ...
      opticflow_got_result[idx_camera][0][0] = false;
    }
  }
  pthread_mutex_unlock(&opticflow_mutex);
}
...
struct image_t *opticflow_module_calc(struct image_t *img, uint8_t camera_id)
{
  ...
  // PRINT("%p:", &cropped_img);
  // PRINT("\n\n");
  sections_img_p = sections_img(img, sections_img_p, NUM_VER_SEC, NUM_HOR_SEC);
  for (int i=0;i<NUM_VER_SEC;i++) {
    for (int j=0;J<NUM_HOR_SEC;j++) {
      ...
      if(opticflow_calc_frame(&opticflow[camera_id], sections_img_p[i+j], &temp_result[camera_id])){
        // Copy the result if finished
        pthread_mutex_lock(&opticflow_mutex);
        opticflow_result[camera_id][i][j] = temp_result[camera_id];
        opticflow_got_result[camera_id][i][j] = true;
        pthread_mutex_unlock(&opticflow_mutex);
      }
    }
  }
