...
static struct opticflow_result_t opticflow_result_matrix[ACTIVE_CAMERAS][num_ver_sec][num_hor_sec];
...
struct image_t *opticflow_module_calc(struct image_t *img, uint8_t camera_id)
{
  ...
  section_width = img->w/num_ver_sec;
  section_height = img->h/num_hor_sec;  

  static struct opticflow_result_t temp_result[ACTIVE_CAMERAS]; // static so that the number of corners is kept between frames

  for (i=0;i<num_ver_sec;i++) {
    for (j=0;i<num_hor_sec;j++) {
        x1_pixels = i*section_width;
        x2_pixels = (i+1)*section_width; 
        y1_pixels = j*section_height;
        y2_pixels = (j+1)*section_height; 
        crop_image (img, x1_pixels, x2_pixels, y1_pixels, y2_pixels)
        
        if(opticflow_calc_frame(&opticflow[camera_id], img_divided, &temp_result[camera_id])){
        // Copy the result if finished
        pthread_mutex_lock(&opticflow_mutex);
        opticflow_result_matrix[camera_id][i][j] = temp_result[camera_id];
